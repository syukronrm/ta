...
//          rawObject.points.foreach(p => node.tree.remove(p))
    ...
    val skyProbU = if (ENV == "GENERATE") {
      ...
      val pddTree = new RTree(new Point3d.Builder, 2, 8, RTree.Split.AXIAL)
...
//      }
  ...
  def findPDDOverlappedObjects(node: Node, rect: Rect3d, currentId: Int): Set[Object] = {
    val tree = node.tree
    val pddBox: Rect3d = rect.getPDD.asInstanceOf[Rect3d]
    val overlappedPoints: Array[Point3d] = new Array[Point3d](N_POINTS)
    tree.search(pddBox, overlappedPoints)

    val objectIds = overlappedPoints.toList.filter(_.isInstanceOf[Point3d]).map(_.o).toSet - currentId
    //println("    rect " + rect.toString)
    //println("    PDRBox " + PDRBox.toString + " objects IDs = " + objectIds.toString)
