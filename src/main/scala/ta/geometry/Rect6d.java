...
public final class Rect6d implements HyperRect {
    private Point6d min, max;

    public Rect6d(List<Point6d> points) {
        setPoints(points);
    }

    Rect6d(final Point6d p) {
        min = new Point6d(p.x, p.y, p.z, p.a, p.b, p.c);
        max = new Point6d(p.x, p.y, p.z, p.a, p.b, p.c);
    }

    public Rect6d(final double x1, final double y1, final double z1, final double a1, final double b1, final double c1, final double x2, final double y2, final double z2, final double a2, final double b2, final double c2) {
        min = new Point6d(x1, y1, z1, a1, b1, c1);
        max = new Point6d(x2, y2, z2, a2, b2, c2);
    }

    Rect6d(final Point6d point1, final Point6d point2) {

        final double minX, maxX, minY, maxY, minZ, maxZ, minA, maxA, minB, maxB, minC, maxC;
        ...
        min = new Point6d(minX, minY, minZ, minA, minB, minC);
        max = new Point6d(maxX, maxY, maxZ, maxA, maxB, maxC);
    }

    @Override
    public HyperRect getMbr(final HyperRect r) {
        final Rect6d r2 = (Rect6d)r;
        ...
        final double minC = Math.min(min.c, r2.min.c);
        ...
        final double maxC = Math.max(max.c, r2.max.c);

        return new Rect6d(minX, minY, minZ, minA, minB, minC, maxX, maxY, maxZ, maxA, maxB, maxC);
    ...
    public HyperPoint getCentroid() {
        ...
        final double dc = min.c + (max.c - min.c)/2.0;

        return new Point6d(dx, dy, dz, da, db, dc);
    }
    ...
    public double cost() {
        ...
        final double dc = max.c - min.c;
        return Math.abs(dx*dy*dz*da*db*dc);
    }
    ...
    public HyperRect getPDR() {
        ...
        final double maxC = Double.MAX_VALUE;
    ...
    public void setPoints(List points) {
        double minX, minY, minZ, minA, minB, minC, maxX, maxY, maxZ, maxA, maxB, maxC;
    ...
    public boolean equals(Object o) {
        ...
        final Rect6d rect6d = (Rect6d) o;

        return min.equals(rect6d.min) &&
                max.equals(rect6d.max);
    }
    ...
    public final static class Builder implements RectBuilder<Rect6d> {

        @Override
        public HyperRect getBBox(final Rect6d rect) {
            return rect;
        }

        @Override
        public HyperRect getMbr(final HyperPoint p1, final HyperPoint p2) {
            return new Rect6d(p1.getCoord(0), p1.getCoord(1), p1.getCoord(2), p1.getCoord(3), p1.getCoord(4), p1.getCoord(5), p2.getCoord(0), p2.getCoord(1), p2.getCoord(2), p2.getCoord(3), p2.getCoord(4), p2.getCoord(5));
        }
    }
}